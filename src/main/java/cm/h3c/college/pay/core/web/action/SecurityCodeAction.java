...
@Namespace("/security")
public class SecurityCodeAction extends GenericAction {

	private Logger log = Logger.getLogger(SecurityCodeAction.class);
	
	private static final long serialVersionUID = 8466699180022373635L;
	...
	public String buildCode() throws IOException {
		...
		// 产生的验证码(4位数字)
		String code = SecurityCodeUtil.getSecurityCode();
		for (int i = 0; i < 4; i++) {
		...
		// 将认证码存入SESSION
		ActionContext.getContext().getSession().put(SystemConfig.SECURITY_CODE_KEY, code);
